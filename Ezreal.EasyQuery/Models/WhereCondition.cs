...
namespace Ezreal.EasyQuery.Models
{
    public class WhereCondition
    {
        ...
        private static readonly MethodInfo _objectToStringMethod =
            typeof(object).GetMethod(nameof(object.ToString), new Type[] { });
        ...
        {
            var member = Expression.PropertyOrField(parameter, this.ColumnName);
            ...
            switch (this.MatchMode)
            {
                ...
                {
                    //member==constant
                    var constant = ConstantExpression(ChangeValueTypeToMemberType(member.Type, this.ColumnValue));
                ...
                case EnumMatchMode.Like:
                {
                    ...
                    var constant = ConstantExpression(this.ColumnValue?.ToString());
                    var currentExpression = member.Type == typeof(string)
                        ? (Expression) member
                ...
                case EnumMatchMode.Between:
                {
                    ...
                    if (this.ColumnValue is object[] valueArray)
                    {
                        var constantStart = ConstantExpression(ChangeValueTypeToMemberType(member.Type, valueArray[0]));
                        var constantEnd = ConstantExpression(ChangeValueTypeToMemberType(member.Type, valueArray[1]));
                ...
                case EnumMatchMode.In:
                {
                    ...
                    {
                        var constant = ConstantExpression(ChangeValueTypeToMemberType(member.Type, item));
